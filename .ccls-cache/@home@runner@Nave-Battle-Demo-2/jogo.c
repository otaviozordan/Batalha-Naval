...
void realizarJogadaAdversario(int tabuleiroJogador[][TAMANHO_TABULEIRO],
  ...
  // prox passo proximo passso da máquina.
    // (0=aleatório); (1=ao redor do anterior aleatoriamente); (2 a direita); (3 a
    // esquerda); (4 a cima); (5 abaixo); (6 regreção para verificação em linha a
    // esquerda); (7 regreção para verificação em linha a direita ); (8 regreção
    // para verificação em coluna a cima); (9 regreção para verificação em coluna
    // a baixo)

  //regreção para direita = ele anda para esquerda(verifica se todas as casas foram marcaadas após andar em sequencia na eaquerda)
  ...
  case 0:
    ...
    linha = rand() % TAMANHO_TABULEIRO;
    coluna = rand() % TAMANHO_TABULEIRO;
    break;// gera um jodada aleatória,
  case 1:
    linha = *ultimaLinha;
    coluna = *ultimaColuna;
    *pos = rand() % 3;
    if (*pos == 0 && coluna + 1 < TAMANHO_TABULEIRO)
      ...
      coluna--;// Move para esquerda
    else if (*pos == 2 && linha + 1 < TAMANHO_TABULEIRO)
      linha++;// Movepara baixo
    ...
    break;//procura o ponto ao redor do ponto inicial
  case 2:
    coluna = *ultimaColuna + 1;
    linha = *ultimaLinha;
    (*rept)++;
    break;// anda para direita após acertar
  case 3:
    coluna = *ultimaColuna - 1;
    linha = *ultimaLinha;
    (*rept)++;
    break;// anda para esquerda após acertar
  case 4:
    linha = *ultimaLinha - 1;
    coluna = *ultimaColuna;
    (*rept)++;
    break;// anda para cima após acertar
  case 5:
    linha = *ultimaLinha + 1;
    coluna = *ultimaColuna;
    (*rept)++;
    break;// anda para baixo após acertar
  case 6:
    linha = *ultimaLinha + (*rept + 2);
    coluna = *ultimaColuna;
    *rept = 0;
    break;// verifica se todas foram marcadas a pós percorrer o tabuleiro pela esquerda
  case 7:
    linha = *ultimaLinha - (*rept + 2);
    coluna = *ultimaColuna;
    *rept = 0;
    break;// verifica se todas foram marcadas a pós percorrer o tabuleiro pela direita
  case 8:
    linha = *ultimaLinha;
    coluna = *ultimaColuna + (*rept + 2);
    *rept = 0;
    break;// verifica se todas foram marcadas a pós percorrer o tabuleiro para cima
  case 9:
    linha = *ultimaLinha;
    coluna = *ultimaColuna - (*rept + 2);
    *rept = 0;
    break;// verifica se todas foram marcadas a pós percorrer o tabuleiro para baixo
  }
...
//baseado na jogada anterior, e se acertou ou errou define a proxima jogada
...
// PARTE 5.2 MOSTRA OS RESULTADOS
